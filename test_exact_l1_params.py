"""Test the EXACT params the L1 pipeline sends, to find the empty-content bug."""
...
for label, kwargs in [
    ("stream=True, no format, no options", {"stream": True}),
    ("stream=True, format='json', no options", {"stream": True, "format": "json"}),
    ("stream=True, format='json', num_predict=2048", {"stream": True, "format": "json", "options": {"num_predict": 2048}}),
    ("stream=False, format='json', num_predict=2048", {"stream": False, "format": "json", "options": {"num_predict": 2048}}),
]:
    print(f"\n--- {label} ---")
    try:
        t0 = time.perf_counter()
        if kwargs.get("stream"):
            stream = client.chat(model=model, messages=messages, **kwargs)
            content = ""
            chunk_count = 0
            for chunk in stream:
                chunk_count += 1
            ...
            print(f"OK in {elapsed:.2f}s ({chunk_count} chunks, {len(content)} chars)")
            print(f"Preview: {content[:300]!r}")
        else:
            r = client.chat(model=model, messages=messages, **kwargs)
            content = r.get("message", {}).get("content", "")
            elapsed = time.perf_counter() - t0
            print(f"OK in {elapsed:.2f}s ({len(content)} chars)")
            print(f"Preview: {content[:300]!r}")
